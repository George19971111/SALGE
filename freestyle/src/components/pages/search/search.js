...
class Search extends Component{
    ...
    componentDidMount(){
      ...
      window.scrollTo(0,0);
      axios.post("http://5.23.48.203:1338/api/posts")
        .then(response=>{
          this.setState({
  ...
  async search(){
    ...
    let arr = [];
    let insta1 = "матрасы диваны кровати"
    let insta2 = "mattresses sofas beds"
    for (var i = 0; i < str.length; i++) {
    str[i].body='';
    }
    await axios.post("http://5.23.48.203:1338/api/posts")
        .then(response=>{
          str1 = response.data
          this.setState({
            loading:false
    ...
    if (result >= 0 || result_desc >= 0 || result_body >= 0){
      var regexp = new RegExp(key, "gi");
        if(str[i].body.search(regexp)<0){
            str[i].body = str1[i].body.substring(0,100)+"..."
          }
          arr.push(str[i])
       }else {
        for(var m = key.length; m > 4; m--){
            ...
            var insta_res2 = await insta2.search(key.toLowerCase(),"gi")
            var result = str[i].title.search(regexp1)
            var result_desc = str[i].title.search(regexp1)
            var result_body = str[i].body.search(regexp1)  
            if (result >= 0 || result_desc >= 0 || result_body >= 0){
  ...
  searching(key,str,str1){
    ...
    for (var i = 0; i < str.length; i++) {
      ...
      for (let j = 0; j < elementos.length ; j++) {
          if (elementos[j].tagName == "PRE") {
            var result_body = elementos[j].innerHTML.search(regexp);
            result1 = str[i].body.search(regexp1)
            if(result_body >= 0 && result1 == -1){
            var sub = (elementos[j].innerHTML.substring(result_body,result_body+ key.length))
            if(result_body==0){
              elementos[j].innerHTML= elementos[j].innerHTML.substring(result_body,160)
            }else if (result_body>0){
              for(let k = result_body; k > 0 ; k--){
              var s = elementos[j].innerHTML.substring(k,k-1)
              if(s=='.' || k==1){
                elementos[j].innerHTML= elementos[j].innerHTML.substring(k,result_body+160)
              }
             }
            }
            elementos[j].innerHTML = elementos[j].innerHTML
            .replace(sub,"<span style='color: red;'>"+sub+"</span>")   
